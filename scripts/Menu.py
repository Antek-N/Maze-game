...
class Menu:
    ...
    def __init__(self) -> None:
        ...
        my_image = pygame_menu.baseimage.BaseImage(
            ...
            drawing_offset=(0, 0))
        self.my_theme = pygame_menu.Theme(background_color=my_image,  # transparent background
    ...
    @staticmethod
    def check_repeat_password(value: str) -> None:
        global password_2
        password_2 = value

    @staticmethod
    def check_nickname(value: str) -> None:
        global nick
        nick = value
    ...
    @staticmethod
    def start(level: int, record_list: list[int], new_record_list: list[int]) -> None:
        if level == 1:
    ...
    def pause(self, level: int, record_list: list[int], new_record_list: list[int],
              is_new_record_list: list[str]) -> None:
        menu = pygame_menu.Menu(height=self.display_height,
                                theme=self.my_theme,
                                title='Pause',
                                width=self.display_width)
        menu.add.button('Play again', self.start, level, record_list, new_record_list)
        menu.add.button('Back to menu', self.result_board, new_record_list, is_new_record_list)
        menu.mainloop(self.display)

    def result_board(self, new_record_list: list[int], is_new_record_list: list[str]) -> None:
        record_string = ""
        for index, i in enumerate(new_record_list):
            record_string += f"level{index+1}: {main.App.to_time(i, 0, 1)}{is_new_record_list[index]}\n"
        menu = pygame_menu.Menu(height=self.display_height,
                                theme=self.my_theme,
                                title='Result board',
                                width=self.display_width)
